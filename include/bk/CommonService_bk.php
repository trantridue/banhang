...
class CommonService {
	...
	function getListMenuByUser($user_id) {
		$listUser = $_SESSION ['session_all_user'];
		for($i = 0; $i < count ( $listUser ); $i ++) {
			if ($user_id == $listUser [$i]->id) {
	...
	function initSessionAllUser() {
		$qry = "select t1.*, REPLACE(group_concat(t3.`key`),',',';') as moduleKey, REPLACE(group_concat(t3.`value`),',',';') as moduleValue
				from user t1
				left join user_module t2 on t2.user_id = t1.id
				left join module t3 on t2.module_id = t3.id
		...
		for($i = 0; $i < count ( $result ); $i ++) {
			...
			$user->moduleKey = $result [$i] ['moduleKey'];
			$user->moduleValue = $result [$i] ['moduleValue'];
			$users [] = $user;
		}
